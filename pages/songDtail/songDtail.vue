<template>
	<view>
		...
		<zaudio theme="theme1" :autoplay="false" :continue="true" ref="zaudio" :getcommentnum="true"></zaudio>
		<!-- 主体 -->
		<view v-if="lyricsIsshow" class="gecilan" :style="{height:mainHeight}" @click="controllshow(false)">
		<bing-lyric :lyrics="lyrics" :centerStyle="centerStyle" :curTime="playinfo.current_value" :areaStyle="cuAreaStyle" :lyricStyle="lyricStyle" ></bing-lyric>
		</view>
		
		 <view v-else @click="controllshow(true)">
		 <image src="../../static/picture/aag.png" :class="{aag:true,aagplay:!paused, aagchange:changePlay,navAll1:isImmersedStatusbar}" mode="aspectFit"> </image>
		...
		<image :src="renderData('coverImgUrl')" mode="aspectFill" :style="" :class="{play:true, playpic:true,playAnimation:!changePlay,playpaused:paused,navAll2:isImmersedStatusbar}"> </image>
		 </view>
		...
		<view class="playnav">
			...
			<image src="../../static/picture/circle.png" mode="aspectFit" class="icon"></image>
			...
			<image src="../../static/picture/list.png" mode="aspectFit" class="icon"></image>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters(['audiolist', 'playIndex', 'playinfo', 'paused','renderIsPlay','audio','renderIndex']),
			renderData() {
				return name => {
		...
		methods:{
			...mapMutations(['set_audio', 'set_playinfo', 'set_pause', 'set_n_pause', 'set_renderIndex','set_audiolist']),
			// 获取歌词
			async getlyric(){
				...
				if(res.data.nolyric||res.data.uncollected){return this.lyrics = []} 
